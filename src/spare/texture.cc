...
//  if (imageSize == 0) {
//    imageSize = width * height * 3;  // 3 : one byte for each Red, Green and
//    Blue component
//  }
//  if (dataPos == 0) {
...
GLuint loadDDS(const char *imagepath) {
  ...
  unsigned int height =
      *(unsigned int *)&(header[8]);  // NOLINT(readability/casting)
  unsigned int width =
      *(unsigned int *)&(header[12]);  // NOLINT(readability/casting)
  unsigned int linearSize =
      *(unsigned int *)&(header[16]);  // NOLINT(readability/casting)
  unsigned int mipMapCount =
      *(unsigned int *)&(header[24]);  // NOLINT(readability/casting)
  unsigned int fourCC =
      *(unsigned int *)&(header[80]);  // NOLINT(readability/casting)
