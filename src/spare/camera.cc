...
void Camera::DrawObject(const GameObject& obj, const glm::vec3& options) {
	...
	glUniform3f(obj.shader->uniform_light_position, light_position.x, light_position.y, light_position.z);
	glUniform3f(obj.shader->uniform_lighting_options, options.x, options.y, options.z);
	...
	glVertexAttribPointer(obj.shader->attrib_position, 3, GL_FLOAT, GL_FALSE, 0, 0);
	...
	glVertexAttribPointer(obj.shader->attrib_tangent, 3, GL_FLOAT, GL_FALSE, 0, 0);

	glEnableVertexAttribArray(obj.shader->attrib_bitangent);
	glBindBuffer(GL_ARRAY_BUFFER, obj.mesh->bitangent_buffer);
	glVertexAttribPointer(obj.shader->attrib_bitangent, 3, GL_FLOAT, GL_FALSE, 0, 0);
